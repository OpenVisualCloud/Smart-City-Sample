...
log = logging.getLogger("count-queue")
...
def quit_service(signum, sigframe):
    log.info("============count-queue: quit_service============")
    global stop
    stop=True
...
while not stop:
    try:
        ...
        print("Exception in count-queue register algorithm: "+str(e), flush=True)
        time.sleep(10)
...
while not stop:
    try:
        ...
        for sensor in dbs.search("sensor:'camera' and status:'idle' and algorithm='queue-counting' and office:["+str(office[0])+","+str(office[1])+"]"):
            try:
                ...
                print("Exception in count-queue search sensor: "+str(e), flush=True)

    except Exception as e:
        print("Exception in count-queue sensor connection: "+str(e), flush=True)
