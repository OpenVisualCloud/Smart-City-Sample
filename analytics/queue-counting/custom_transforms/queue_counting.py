...
class QueueCounting:
    ...
    def init_logging(self):
        self.log = logging.getLogger("queue_counting")
    ...
    def process_frame(self,frame):
        ...
        if len(messages) > 0:
            ...
            json_msg["count"] = {"queue":len(self.identities)}
    ...
    def get_ids_by_embeddings(self, frame):
        ...
        for i in range(len(detected_tensors)):
            for j in range(len(self.identities)):
                distances[i][j] = QueueCounting.compute_reid_distance(detected_tensors[i], self.identities[j]["embedding"])
