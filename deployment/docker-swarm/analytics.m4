...
ifelse(defn(`SCENARIO_NAME'),`traffic',`
    defn(`OFFICE_NAME')_analytics_traffic:
        image: PLATFORM_IMAGE(`smtc_analytics_object_vcac-a_'defn(`FRAMEWORK'):latest)
        volumes:
...
ifelse(defn(`SCENARIO_NAME'),`stadium',`
    defn(`OFFICE_NAME')_analytics_entrance:
        image: PLATFORM_IMAGE(`smtc_analytics_entrance_vcac-a_'defn(`FRAMEWORK'):latest)
        volumes:
            - /etc/localtime:/etc/localtime:ro
PLATFORM_VOLUME_EXTRA()dnl
        environment:
...
PLATFORM_ENV_EXTRA()dnl
    ...
    defn(`OFFICE_NAME')_analytics_crowd:
        image: PLATFORM_IMAGE(`smtc_analytics_crowd_vcac-a_'defn(`FRAMEWORK'):latest)
        volumes:
            - /etc/localtime:/etc/localtime:ro
PLATFORM_VOLUME_EXTRA()dnl
        environment:
