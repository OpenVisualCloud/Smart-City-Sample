...
ifelse(defn(`SCENARIO_NAME'),`traffic',`
    defn(`OFFICE_NAME')_analytics_traffic:
        image: PLATFORM_IMAGE(defn(`DOCKER_REGISTRY')`smtc_analytics_object_'defn(`PLATFORM_SUFFIX')`_'defn(`FRAMEWORK'):latest)
        volumes:
...
ifelse(defn(`SCENARIO_NAME'),`stadium',`
    defn(`OFFICE_NAME')_analytics_entrance:
        image: PLATFORM_IMAGE(defn(`DOCKER_REGISTRY')`smtc_analytics_entrance_'defn(`PLATFORM_SUFFIX')`_'defn(`FRAMEWORK'):latest)
        volumes:
            - /etc/localtime:/etc/localtime:ro
PLATFORM_VOLUME_EXTRA()dnl
        environment:
...
PLATFORM_ENV_EXTRA()dnl
    ...
    defn(`OFFICE_NAME')_analytics_crowd:
        image: PLATFORM_IMAGE(defn(`DOCKER_REGISTRY')`smtc_analytics_crowd_'defn(`PLATFORM_SUFFIX')`_'defn(`FRAMEWORK'):latest)
        volumes:
            - /etc/localtime:/etc/localtime:ro
PLATFORM_VOLUME_EXTRA()dnl
        environment:
