...
for clip in "${CLIPS[@]}"; do
    ...
    if test ! -f "$DIR/$clip_mp4"; then
        ...
        if test "$reply" = "accept"; then
            ...
            docker run --rm -u $(id -u):$(id -g) -v "$DIR:/mnt:rw" -it openvisualcloud/xeon-centos76-media-ffmpeg:19.10.1 ffmpeg -i /mnt/$tmp -c:v libx264 -profile:v baseline -x264-params keyint=30:bframes=0 -c:a aac -ss 0 /mnt/$clip_mp4
            rm -f "$DIR/$tmp"
        else
            echo "Skipping..."
        fi
