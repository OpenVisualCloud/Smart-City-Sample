    ...
    for clip in "${!clipz}"; do
        ...
        if test ! -f "$DIR/archive/$clip_mp4"; then
            ...
            if test "$reply" = "accept"; then
                echo "Downloading..."
                tmp="tmp_$clip_name"
                wget -q -O "$DIR/archive/$tmp" "$url"
                docker run --rm -u $(id -u):$(id -g) -v "$DIR/archive:/mnt:rw" -it ${FFMPEG_IMAGE} ffmpeg -i /mnt/$tmp -c:v libx264 -profile:v baseline -x264-params keyint=30:bframes=0 -c:a aac -ss 0 /mnt/$clip_mp4
